...
function runSearch(q: string): Item[] {
  ...
  return ITEMS
    .map(item => {
      ...
      let score = 0;
      if (t.startsWith(norm))                                       score += 6;
      else if (t.includes(norm))                                    score += 4;
      if (item.tags.some(tag => tag.toLowerCase().startsWith(norm))) score += 3;
      else if (item.tags.some(tag => tag.toLowerCase().includes(norm))) score += 2;
      if (c.includes(norm))                                         score += 1;
      return { item, score };
    })
    .filter(x => x.score > 0)
  ...
  return (
    <section className="bg-[#1f2e91] border-b border-[#0569ff] flex flex-col items-center justify-center px-[20px] py-[60px] w-full min-h-[480px]">
      <div className="flex flex-col gap-[32px] items-center justify-center max-w-[1400px] px-[20px] lg:px-[100px] w-full">
        ...
        {/* Subtitle */}
        <p className="font-['Avenir_LT_Pro:85_Heavy'] text-[26px] leading-[28px] text-white text-center w-full">
          Estamos aqui para ajudar.
        </p>

        {/* Description */}
        <p className="font-['Avenir_LT_Pro:55_Roman'] text-[20px] leading-[26px] text-white text-center w-full max-w-[800px]">
        ...
        <div className="flex flex-wrap gap-[16px] items-center justify-center w-full">
          ...
          <button className="group bg-white border border-white hover:border-[#e2e8f0] hover:bg-[rgba(255,255,255,0.9)] active:bg-white transition-colors
            flex gap-[10px] items-center justify-center
            min-h-[50px] min-w-[190px] overflow-hidden px-[20px] py-[10px] rounded-[8px] cursor-pointer shrink-0">
